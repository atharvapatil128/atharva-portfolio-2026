...
**The Mono Legibility Floor.** IBM Plex Mono never renders below 10px and uses at least weight 500 at that floor. Evidence and operational labels inside the Selected Work deck use the sturdier 12px/16px, weight-600 role with restrained tracking; size alone is not a substitute for optical weight. Dense evidence can become concise or progressively disclosed, but it must not become faint microtype.
...
Desktop hover/focus changes the active column and reveals a compact, evidence-led project brief: role and period are grouped together, followed by verified project evidence, so the expanded surface reads as a case-study preview rather than decoration. The desktop reading column is derived from the viewport rather than the card's animated width, keeping its line breaks stable throughout expansion so the heading travels once instead of wobbling as evidence reflows. Collapsed cards remain quieter but are not empty: the title leads, followed by the project claim and a restrained category/evidence line. Their artwork occupies a consistent media zone with a deliberate gap before the copy; expanded cards preserve an equivalent horizontal gap between the reading column and artifact. The deck is deliberately substantial enough to dominate the section rather than read as a compact index. Mobile and tablet use native horizontal snapping, preserve large touch-sized project canvases, clamp descriptors on complete lines, and leave the next card peeking into view; the denser desktop brief and category/evidence line are omitted there. Project-specific compositions adapt rather than crop mechanically: MEAD's two screens remain fully framed on touch layouts, while Streaming Helper preserves the dashboard's native aspect ratio so expansion reveals the interface instead of magnifying a crop. Each project owns a distinct media grammar rather than sharing one generic thumbnail.
